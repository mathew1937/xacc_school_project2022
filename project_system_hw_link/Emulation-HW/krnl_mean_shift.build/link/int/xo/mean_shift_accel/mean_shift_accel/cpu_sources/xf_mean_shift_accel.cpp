...
#include "../../project/src/xf_cvt_color_config.h"
#include "../../project/src/xf_mean_shift_config.h"
...
// clang-format off
    ...
    xf::cv::Mat<XF_8UC3, XF_HEIGHT, XF_WIDTH, XF_NPPC1> inMat(rows, cols, img_inp);
	//#pragma HLS stream variable=inMat.data depth=2

    xf::cv::Mat<XF_8UC3, XF_HEIGHT, XF_WIDTH, XF_NPPC1> outMat(rows, cols, img_inp);
	//#pragma HLS stream variable=outMat.data depth=2

    xf::cv::rgb2hsv<XF_8UC3, XF_HEIGHT, XF_WIDTH, XF_NPPC1>(inMat, outMat);
    xf::cv::MeanShift<XF_MAX_OBJECTS, XF_MAX_ITERS, XF_MAX_OBJ_HEIGHT, XF_MAX_OBJ_WIDTH, XF_8UC3, XF_HEIGHT, XF_WIDTH,
                      XF_NPPC1>(outMat, tlx, tly, obj_height, obj_width, dx, dy, track, frame_status, no_objects,
                                no_of_iterations);
	}
}
